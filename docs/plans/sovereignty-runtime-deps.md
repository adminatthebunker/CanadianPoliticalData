...
| 7 | `represent.opennorth.ca/postcodes/` live call on the homepage "who represents me" widget (`services/api/src/routes/lookup.ts`) | **Done — 2026-08-02.** Was a hardcoded, uncached fetch on a hot user path (5xx → user-facing 503); the route predates the cached path and never made it onto this tracker. Rewritten local-first: `resolvePostcode()` (30-day SWR `postcode_cache`) → point-in-polygon against our own `constituency_boundaries` mirror → join `politicians` on `constituency_id` (same `{set}/{slug}` scheme both sides; name+level fallback for rows without one). Only remaining upstream call is the cached centroid geocode, shared with the public postcodes API. Response shape unchanged; `x-postcode-source` header exposes cache state. Spot-checks: K1A 0A6 (4 reps incl. ward councillor + mayor via city polygon, 41 ms, cache); H3Z 2Y7 result byte-equivalent to Open North's own representatives output. | ~~Longer-term: a local postcode-centroid table (StatCan PCCF — the `source` column in migration `0055` is the seam) would remove the last upstream call.~~ ⛔ **PCCF was the wrong nominee** — it is discontinued as a public product, DLI/academic-only, and carries Canada Post licensed data. The `0055` `source`-column seam is still exactly right; the source that goes in it is the **National Address Register**, not PCCF. <br><br>**Research complete 2026-08-18 — see [`../research/boundaries/`](../research/boundaries/).** ⛔ Open North has been **down since 2026-08-07** (cert `CN=represent-tempeh.opennorth.ca` expired; origin 502), and `postcode_cache` held only **8 rows**, so the SWR fallback was decorative — every uncached postcode 503s. **(a) The geocode:** StatCan PCCF is discontinued/DLI-only and Canada Post-encumbered; the open replacement is the **StatCan National Address Register** (cat. 46-26-0002, Statistics Canada Open Licence, explicit right to distribute and sublicence), deriving 6-char centroids at an estimated 73–92% of active postal codes, with the free 2021 FSA Boundary File as a bounded fallback. Recommendation: once local, **remove the outbound call from the hot path entirely** rather than keeping Open North as primary — otherwise the next outage reproduces this incident. **(b) The boundaries were never really ours either:** all 1,949 rows are Open North mirrors and we inherited their staleness — measured, **9.15% of Canadian addresses get no provincial answer today** and 12.61% get a wrong or absent one across the six jurisdictions where authoritative geometry could be checked. See [`../research/boundaries/impact.md`](../research/boundaries/impact.md). Build plan not yet written; 99 handoff items triaged in [`../research/boundaries/overview.md`](../research/boundaries/overview.md) § Handoff register. |
