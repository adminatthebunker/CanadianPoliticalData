...
- ✅ **Applied 2026-04-30:** 0018 (votes + vote_positions). NT consensus-government data validated the schema without revisions (`vote_type='consensus'` + nullable tallies + empty `vote_positions` is the schema-anticipated shape). Federal openparliament.ca extraction shipped same day with 99.98% politician-FK across 18.5 years.
...
- **Speeches coverage:** federal + QC + AB + BC + MB + NS + NB + NL Hansard ingested — **2,568,359 speeches, 3,398,197 chunks, 100 % embedded** with Qwen3-Embedding-0.6B. MB Hansard now spans legs 37-43 (1999-11 → 2026-04, 407k speeches across 2,325 sittings) via an era-dispatching parser that handles both modern MsoNormal and Word-97 HTML export. ON / NT / NU / SK / PE / YT Hansard pipelines are the remaining build-out.
