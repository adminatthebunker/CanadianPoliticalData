...
- **Cross-jurisdictional Tier-2 attribution** is a layered pipeline that picks up rows the chamber parsers leave with `politician_id IS NULL`. Four passes are live across 2026-05-05 → 2026-05-14:
    - **Pass 1 — inline-name presiding-officer resolver** (commit `bb41d88`, 2026-05-05): extracts names from parens-bearing presiding labels (`The Deputy Speaker (Mr. Bas Balkissoon)` / `Madam Clerk (Patricia Chaychuk)` / `La Vice-Présidente (Mme Gaudreault)`) and FK-matches against politicians within the same province (ON +862 / MB +1,961 / QC +16, daily 23:30 UTC schedule).
    - **Pass 2 — date-windowed Deputy disambiguation** (2026-05-06, QC slice): fourth narrowing step at Pass-1's tie-break — date-windowed presiding-role membership intersected with surviving surname candidates. `DEPUTY_PRESIDING_ROSTER["QC"]` (22 entries, 37L→43L Vice-Présidents). QC parens-attributed surface 67.4% → 96.7% (+21,223 speeches).
    - **Pass 3 — role-only presiding** (2026-05-06 → 2026-05-14, AB/BC/MB/SK): handles labels with no inline name (`The Deputy Speaker`, `deputy_speaker`). New `ROLE_ONLY_PRESIDING_ROSTER` + `ROLE_ONLY_OFFICE_MAP` + `resolve_role_only_presiding`. AB Deputy Speaker + Deputy Chair (full 1993→present); BC Deputy Speaker 39L→43L; MB Deputy Speaker 37L→43L (6 entries); SK Deputy Speaker 28L3S→30L (4 entries). Confidence 0.85. Daily schedules at 14:50 / 15:50 / 17:50 / 22:35 UTC.
    - **Pass 4 — cross-jurisdictional named-speaker resolver** (commit `59f0c7a`, 2026-05-06): three confidence buckets (0.85 full-name / 0.80 surname-single / 0.75 date-windowed). Pre-filters vocatives + parliamentary staff. ~30K speeches resolved across MB / AB / ON / NB / BC / NS / NL.
    - **BC ALL-CAPS resolver** (commit `b0b2562`, 2026-05-05) is a Pass-1 sibling for `MR. G.S. WALLACE (Oak Bay)` / `HON. D. BARRETT (Premier)` shapes (P29-P34 era, +11,017 attributions).
    - **ON Speaker historical roster + MB chamber-parser hygiene** (commit `bfeb5a3`, 2026-05-14): SPEAKER_ROSTER["ON"] 1 → 12 entries (32L-44L) closing the pre-2008 bare-`The Speaker` bucket (+46,415, ON 87.54% → 92.45%). MB `_ROLE_PATTERNS` extended to catch `Mr./Mrs./Ms./Madam` honorific-prefixed presiding forms + new `relink-mb-speaker-roles` Click command; MB Deputy Speaker pre-43L roster expansion. MB 86.67% → 89.37%.
  - **Remaining rotating-role buckets are deferred** by design: AB Acting Speaker / Chair (~30K), BC Chairman / The Chair (~24K), MB Chairperson (~10.9K) — chamber Hansards lack in-corpus chair-handover signals that the date-windowed shape can resolve.
