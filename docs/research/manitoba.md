...
- **MB parliamentary-staff role-tagging follow-up (2026-05-20):** the prior cycle closed the politician-bearing empty-role bucket but left ~2,109 `speech_type='staff'` rows with `speaker_role IS NULL` because `_ROLE_PATTERNS` had only the bare `^the\s+clerk\s*$` / `^the\s+sergeant[-\s]at[-\s]arms\s*$` patterns — they didn't match `Madam Clerk (Patricia Chaychuk)` / `Deputy Clerk (Mr. Rick Yarish)` / `Madam Clerk Assistant (Monique Grenier)` / `Acting Sergeant-at-Arms (Mr. Craig Waterman)` etc. The two narrow patterns consolidated into 5 parens-aware patterns covering Clerk Assistant / Deputy Clerk / Clerk / Deputy Sergeant-at-Arms / Sergeant-at-Arms, each with `(?:the\s+|madam\s+|mr\s+)?(?:acting\s+)?` honorific prefixes and `(?:\s+\S.*)?` trailing-name tails (after `_norm()` replaces parens with whitespace via `re.sub(r"[^\w\s-]", " ", t)`, a parens-name suffix becomes trailing text). Pattern order is significant — `clerk assistant` before `deputy clerk` before `clerk` so iteration in `_match_role()` picks the more-specific role first. Daily `relink-mb-speaker-roles` cron at 17:40 UTC auto-applied the new patterns. **Outcomes:** 2,109 rows tagged (`The Clerk: 1,586 / The Deputy Clerk: 335 / The Clerk Assistant: 93 / The Deputy Sergeant-at-Arms: 87 / The Sergeant-at-Arms: 8`); `politician_id` stays NULL by design — these are parliamentary staff, not MLAs; what changed is the role tag, which makes coverage queries honest about *unattributed-and-untagged* vs. *unattributed-by-design*.
...
- **Remaining gap (~10.6% unresolved, characterized 2026-05-20):** the 43,480 MB rows without `politician_id` break down as: **29,320** `Honourable Members` chorus turns (`speech_type='group'`, by-design-NULL — "hear, hear" acknowledgements aren't attributable to an individual); **10,909** `The Chairperson` rotating Committee-of-the-Whole turns + **151** `The Deputy Chair` + **133** `The Acting Speaker` (deferred 2026-05-20 — probe confirmed MB Hansard does NOT carry "I now invite X to take the chair" / "elected as Chairperson" announcement shapes; only 3 of 11,170 Chairperson rows contain anything resembling chair-handover language, and the matches are content references not announcements — same family as deferred AB Acting Speaker / BC Chairman; defer-decision captured as a comment block above `ROLE_ONLY_PRESIDING_ROSTER["MB"]` in `presiding_officer_resolver.py:713`); **2,109** parliamentary-staff rows (`speech_type='staff'`, now role-tagged but never get `politician_id` because clerks/sergeants aren't MLAs); **625** committee-section headings (`speech_type='metadata'`, correctly classified as non-speech); **224** floor edge cases (constituency-fragment parens like `(Radisson)`, long witness intros, multi-paragraph speaker-name shapes that the parser truncated — would need a multi-paragraph parser refactor; deferred). Re-evaluate the Chairperson bucket if an external roster source emerges (Manitoba Assembly Journals?).
