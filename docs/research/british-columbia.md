...
- [x] **Historical backfill — bills: CLOSED AS COMPLETE 2026-07-31 (nothing further ingestible).** The April note "PDMS serves every session back to 1872" was a misreading: GraphQL `allSessions` carries session METADATA to 1872-02-15 (174 sessions), but per-bill DATA starts at P35/1992 — verified three ways: `pdms/bills/progress-of-bills/{sessionId}` AND `pdms/bills/{sessionId}/{readingTypeId}` both return `[]` for every pre-P35 session probed (1872/1903/1941/1972/1987); no bill files exist in the `/ldp/{parl}{sess}/1st_read/` tree pre-P35 (served via `lims.leg.bc.ca/pdms/file{path}` or `www.leg.bc.ca/hansard-content{path}`); the search index holds 4,161 `Bills`-typed docs, and pre-P35 parliaments' documents are Journals/Hansard/Index only (3–5 scanned sessional volumes per parliament — pre-1992 bills live only inside those PDFs; recovering them would be an OCR project, not a bills pipeline). **We hold all 2,284 bills across P35–P43 = the full digital record.** Search-API caveat discovered en route: `POST /search`'s `contentTypes` filter silently no-ops for some values (returned the whole 124K index for `["Bills"]` while working for `["Hansard Committees"]`) — never trust a census from it without checking the `metadata_file_content_type` facet of the response.
- [x] **✅ Votes & Proceedings divisions SHIPPED 2026-07-31 (same day as discovery, commit `4f543f5`):** `ingest-bc-vp-votes` landed **1,412 recorded divisions (1992-03-23 → 2026-05-28) with 77,055 vote_positions at 97.8% politician-FK** — BC is the first province with per-member roll-call data. 770 divisions bill-linked; 0 fetch failures; every vote carries a result. Era traps handled (Nays-first header order on defeated amendments, 1996 cell-boundary name fusing, "deferred division" phrasing, tally-implied verdicts, dotted multi-initials vs "de Jong"-style particles). Daily 14:40 UTC schedule (`since_days: 14`). **Resolution FINAL 2026-07-31 after the P35–38 roster-patch arc: 99.68%** (76,806 / 77,051). The patch surfaced and fixed FOUR distinct rot layers: (1) LIMS `allMemberParliaments` holes for partial-term members (died/resigned/by-election — 36 gap terms via Wikipedia lists + hand-verified Gingell/Hurd/Brewin/Reitsma, incl. the Judith-Reid-entered-by-1998-by-election correction that de-ambiguates bare "Reid"); (2) Wikipedia-matcher nickname duplicates (matcher hardened with ~60 nickname folds + unique-surname+initial fallback; 17 dups reverted); (3) **pre-existing formal-name duplicate politicians of original P29–34-run vintage** (Arthur Daniel Miller/Dan Miller, Moe Sihota, Stan Hagen, Dale Lovick, Jack Weisgerber, Harry De Jong — carried 530 misattributed speeches now moved to canonicals); (4) **a LIMS upstream data error**: its own edges assign Bernie Simpson's P35 service to Bob Simpson's record (both "B. Simpson"; Bernie absent from LIMS entirely; wrong term deleted locally). Plus V&P-side bugs fixed en route: roster cache was per-parliament keyed off the unsorted listing's first sitting (now per sitting date), no-space initials ("B.Jones"), hyphen↔space surnames, NBSP-in-name. **Honest remaining unresolved (~245): P35 bare "de Jong" (Harry + Michael sat simultaneously — genuinely ambiguous, NULL is correct), a few NBSP-variant strays, upstream misspellings.** Cross-province lesson: the formal-vs-colloquial duplicate layer likely exists in other provinces' Wikipedia-era rosters — audit candidate. (b) **legacy-regex supersession RESOLVED 2026-07-31: NO ACTION — the sources are complementary, not overlapping.** Empirical audit of the 305 `votes-bc` rows (1970→2026): all 42 tally-bearing rows are **pre-1992 recorded divisions** (the only division data before V&P's floor); all 177 post-1992 rows are consensus-shape voice-vote notations (clause approvals, "Motion approved") — a different event class than recorded divisions. Zero (date, ayes, nays) collisions with V&P; same-day motion comparisons show distinct motions (sometimes opposite results). Both sources stay: `votes-bc` owns pre-1992 divisions + consensus notations, `votes-bc-vp` owns recorded divisions 1992+. Deferred nicety: fuzzy-matching same-motion pairs to give V&P votes Hansard `speech_id` anchors.
- ~~[ ] **★ Votes & Proceedings — RECORDED DIVISIONS WITH NAMES (found 2026-07-31, not yet ingested).**~~ `GET lims.leg.bc.ca/pdms/votes-and-proceedings/{parl}{sess}` returns a per-sitting structured listing (`fileName`/`filePath`/`date` + `votesAttributesByFileId.voteNumbers` — recorded-division numbers per sitting!); documents at `pdms/file{filePath}/{fileName}` (e.g. `/ldp/42nd4th/votes/v231109.htm`) carry **full division records with member surnames per side** ("Yeas — 53 ‹names›… Nays — 23 ‹names›"). ~2,224 V&P docs in the search census. This upgrades BC votes from Hansard-regex consensus-shape (no positions) to real recorded divisions with `vote_positions`. 34th session returns empty — floor probably P35, matching bills. Natural next BC slice.
