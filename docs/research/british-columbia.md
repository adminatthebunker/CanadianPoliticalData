...
- [x] **✅ Votes & Proceedings divisions SHIPPED 2026-07-31 (same day as discovery, commit `4f543f5`):** `ingest-bc-vp-votes` landed **1,412 recorded divisions (1992-03-23 → 2026-05-28) with 77,055 vote_positions at 97.8% politician-FK** — BC is the first province with per-member roll-call data. 770 divisions bill-linked; 0 fetch failures; every vote carries a result. Era traps handled (Nays-first header order on defeated amendments, 1996 cell-boundary name fusing, "deferred division" phrasing, tally-implied verdicts, dotted multi-initials vs "de Jong"-style particles). Daily 14:40 UTC schedule (`since_days: 14`). **Known residue:** (a) ~1,700 unresolved positions concentrated in P36 roster gaps — Gingell/Hurd/Brewin missing P36 `politician_terms` rows, **Paul Reitsma missing from `politicians` entirely**, Linda-vs-Judith Reid ambiguous from bare surnames (1996-era docs print no initials); raw names retained for re-resolution after a roster patch; (b) the 250 legacy Hansard-regex `votes-bc` rows overlap V&P divisions for the same events — supersession decision pending (operator).
