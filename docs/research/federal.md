...
- **Difficulty (1–5):** **2**.
- **Notes:** Discovery filters on `has_evidence=true` to skip meetings pending transcription (transcripts publish on irregular delays after meetings; the 14-day forward-incremental overlap re-visits recent meetings until evidence lands). `--include-in-camera` flag exists but rarely useful (in-camera meetings almost never have evidence published). → `~/.claude/plans/hay-dude-i-know-bubbly-sun.md`
- **✅ Historical backfill SHIPPED 2026-07-27** via new `--all-sessions` walker (mirrors `ingest-federal-bills --all-sessions`: iterates `legislative_sessions` federal rows, `federal_session_bounds()` date-window per session — the 896k-mis-tagging guardrail per iteration). Full-walk outcome: **2,740,850 speeches inserted / 15,643 meetings** across P39-S1 → P45-S1; P35-P38 correctly return 0 (openparliament's committee-evidence floor is P39). Per-session: P39-S1 245K, P39-S2 143K, P40-S1 0.5K (short session), P40-S2 169K, P40-S3 209K, P41-S1 272K, P41-S2 178K, P42-S1 533K, P43-S1 83K, P43-S2 166K, P44-S1 563K, P45-S1 177K. Federal committee corpus 35K → **2.78M**, oldest 2006-04-06, 67.2% MP-FK (remainder = witnesses, by design). **Systemic finding:** the daily `since_days=14` job had permanently missed evidence published >14 days after its meeting — P44-S1 (+563K) and P45-S1 (+177K) were mostly *that*, not deep history. The `--all-sessions` walk (or a targeted current-session no-since run) is the periodic catch-up for late-published evidence; consider a monthly current-parliament full-session re-walk schedule.
...
- [x] **Committee transcripts (evidence) — live 2026-05-18.** `ingest-federal-committees` daily 11:20 UTC; 5,471 P45-S1 speeches on first ingest; `committees_status='live'`.
- [x] **Committee historical backfill (P39-P45) — shipped 2026-07-27** via `--all-sessions` walker: 2.74M speeches / 15,643 meetings; corpus 35K → 2.78M. See § Committee Activity notes (incl. the >14-day late-evidence gap the walker also closed).
