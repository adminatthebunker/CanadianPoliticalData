...
**Status snapshot (2026-05-21):** ✅ **Bills live** for Legislature 31, sessions 1+2 (114 bills / 551 events / 114 sponsors / **100% FK-linked**). ✅ **Hansard live** via PDF pipeline — 439,125+ speeches (2000-02-17 → 2026-05-14), 487,221+ chunks, 100% Qwen3-embedded. ✅ **Speaker (presiding officer) resolution live** — 114,450 'The Speaker' rows + 19,363 'The Deputy Speaker' + 11,631 'The Deputy Chair' tied to historically-correct officeholders by date. ✅ **Committee transcripts live (2026-05-19)** — first provincial-committee pipeline; 9,988 speeches across 60 L31-S2 meetings via `ingest-ab-committees`; substitute-MLA detection via PDF cover-page roster. Committee memberships pre-existing via `committees.py:ingest_ab_committees`. ✅ **Speech-type honest-coverage classification (2026-05-21):** AB chamber-Hansard parser now emits `speech_type='group'` for `Hon. Members` / `Some Hon. Members` chorus turns (4,896 rows) and `speech_type='staff'` for `The Clerk` / `The Clerk Assistant` / `The Sergeant-at-Arms` turns (1,296 rows) — mirrors the QC / NL parser `_GROUP_ROLES` / `_STAFF_ROLES` role-set pattern; existing rows backfilled in one transaction. The variant spelling `The Sergeant At Arms` (one row, no hyphens) is canonicalized to the hyphenated form via parser fix. Attribution % unchanged (these were never MLAs); honest-coverage signal corrected so search filters / coverage queries can exclude non-politician turns from the unattributed-politicians denominator. Rotating-role buckets (`The Acting Speaker` / `The Chair` / `The Chairman`) stay `floor` — no in-corpus chair-handover signal, deferred. Chamber speaker resolution still gated by the roster gap — only 91 AB politicians are in `politicians` against a 26-year corpus (~57% of speeches remain `politician_id IS NULL`); historical MLA enrichment is the outstanding work, not a resolver bug.
...
- **✅ Historical backfill L29-L30 SHIPPED 2026-07-27.** Per-session `ingest-ab-committees --legislature N --session S` invocations (no walker flag needed — the listing URL already templates legislature/session). All 8 sessions ingested: **145,682 speeches / 602 meetings** (L29: 83,651 across S1-S4; L30: 62,031 across S1-S4). MLA-FK 23-36% per session (witness-heavy as expected; cover-page roster parser resolved ~85-90% of roster names per session, augmenting the current-era `politician_committees` lookup). The current 11 standing-committee acronyms empirically covered L29/L30 — a pre-run probe of all 88 (legislature, session, acronym) listing combos found 64 populated; CI/CEB correctly absent (didn't exist yet), PE/MS/PB spotty (met rarely). Two drive-by fixes: invalid `hhmm` token in one upstream PDF filename (`20191119_0090_01_rs.pdf`) crashed the whole RS L30-S1 listing — now warns + defaults 00:00; `pt.constituency` → `pt.constituency_id` column bug in the ambiguous-name disambiguator (latent since ship — only fires on surname+first-token ambiguity), including slug-vs-name normalization for opennorth-style `constituency_id` values.
- **Pending follow-ups:** select-special + completed-mandate committees not yet enumerated (only the 11 standing committees are hardcoded); pre-L29 sessions unprobed.
