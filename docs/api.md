...
| `GET /postcodes/:postcode` | free | read:public | Resolve postcode (or 3-char FSA) to lat/lng + `boundaries.{federal,provincial,municipal}` (byte-identical shape to `/boundaries/lookup` for parser reuse). Resolved from the StatCan National Address Register (`public.postcode_centroids`); `source` is `nar | cache | fsa_derived` |
...
**Postcodes — StatCan National Address Register with our own PIP.** ⛔ This was an Open North proxy until 2026-08-07, when that host's TLS certificate expired and its origin began 502ing for eleven days — the cache held eight rows, so effectively every Canadian postal code returned 503. Rebuilt on `public.postcode_centroids` (migration `0062`, from the StatCan NAR via `ingest-nar-postcodes`). ⚠ Open North was deliberately NOT retained as a fallback: a fallback exercised only during an outage is a fallback that fails during one. Resolution order is `postcode_centroids` → `postcode_cache` → FSA-average → not_found, surfaced as `source: nar | cache | fsa_derived`. `postcode_cache` (`0055`) is now the RETAINED-OVERRIDE layer for codes NAR cannot supply — PO-box-only, rural-route-only and large-volume-receiver codes (`K1A 0A6` has zero NAR rows). The centroid is fed to `lookupBoundariesAtPoint()` for the `boundaries.*` slot so callers can reuse their `/boundaries/lookup` parser. ⚠ `approximate` is load-bearing on an FSA-derived answer: 33–71% of FSAs cross a federal riding boundary.
