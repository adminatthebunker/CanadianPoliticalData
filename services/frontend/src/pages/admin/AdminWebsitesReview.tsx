...
interface CoverageResp {
  total_active: number;
  with_any_website: number;
  by_source: { source: string; n: number; flagged: number }[];
  by_label: { label: string; n: number; flagged: number }[];
}
...
export default function AdminWebsitesReview() {
  useDocumentTitle("Admin · Websites Review");
  const [labelFilter, setLabelFilter] = useState<string>("");
  const [busy, setBusy] = useState<string | null>(null);

  const cov = useAdminFetch<CoverageResp>("/websites/coverage", { pollMs: 10000 });
  const path = labelFilter
    ? `/websites/flagged?label=${encodeURIComponent(labelFilter)}&limit=100`
    : "/websites/flagged?limit=100";
  const flagged = useAdminFetch<FlaggedResp>(path);

  async function approve(id: string) {
    setBusy(id);
    try {
      await adminFetch(`/websites/${id}/approve`, { method: "POST" });
      flagged.refresh();
      cov.refresh();
    } catch (e) {
  ...
  async function reject(id: string) {
    if (!confirm("Delete this website row? This cannot be undone.")) return;
    setBusy(id);
    try {
      await adminFetch(`/websites/${id}/reject`, { method: "POST" });
      flagged.refresh();
      cov.refresh();
    } catch (e) {
    ...
    <div className="admin__content">
      {coverage && (
        <div className="admin__stats-grid">
          ...
          />
          <Stat
            label="With ≥1 website"
            value={coverage.with_any_website.toLocaleString()}
            sub={coverage.total_active > 0
              ? `${Math.round(100 * coverage.with_any_website / coverage.total_active)}% coverage`
              : undefined}
          />
          <Stat
            ...
            sub="agent_sonnet rows below 0.85 threshold"
          />
        </div>
      )}
      ...
      <section className="admin__section">
        <div className="admin__toolbar">
          <label>
            ...
            <select value={labelFilter} onChange={e => setLabelFilter(e.target.value)}>
              ...
              {coverage?.by_label.map(p => (
                <option key={p.label} value={p.label === "<null>" ? "" : p.label}>
                  {p.label} ({p.flagged} flagged)
                </option>
              ))}
            </select>
          </label>
        ...
        {items.length === 0 && !flagged.loading && (
          <p className="admin__empty">
            No flagged rows{labelFilter ? ` for ${labelFilter}` : ""} — queue is clear.
          </p>
        )}
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    {safeHttpHref(r.url) ? (
                      ...
                      <code title="non-http(s) URL — not rendered as a link">{r.hostname || r.url}</code>
                    )}
                  </td>
