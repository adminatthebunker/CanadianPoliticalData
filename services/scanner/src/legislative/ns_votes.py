"""NS votes extractor — Hansard-text regex over already-ingested NS speeches.

Pattern coverage (probe 2026-04-30): 2,781 inline motion outcomes,
182 division calls, 2,601 question-call phrases, 0 numerical tallies.
Predominantly consensus-shape.

Idempotency: `(source_system='votes-ns', source_url)`.
"""
...
def _find_bill_id_for(raw: dict, body: str, idx: dict[str, str]) -> Optional[str]:
    if not idx:
        return None
    topic = (raw or {}).get("ns_hansard", {}).get("topic") or ""
...
async def _load_bill_index(db: Database) -> dict[str, str]:
    rows = await db.fetch(
        ...
        "WHERE ls.province_territory='NS' AND ls.level='provincial'"
    )
    out: dict[str, str] = {}
    for r in rows:
...
async def _upsert_vote(db: Database, *, speech_row: dict, detected: _Detected,
    ...
    result = await db.fetchrow(
        ...
        VALUES ($1::uuid, 'provincial', 'NS', $2, $3::uuid, $4, $5, $6,
...
async def extract_ns_votes(db: Database, *, limit_sittings: Optional[int] = None) -> IngestStats:
    stats = IngestStats()
    bill_index = await _load_bill_index(db)
    log.info("ns_votes: loaded %d NS bill references", len(bill_index))
    where_sittings = ""
    if limit_sittings is not None:
        where_sittings = f"""
        AND s.spoken_at::date IN (SELECT spoken_at::date FROM speeches
          WHERE source_system='hansard-ns' GROUP BY 1 ORDER BY 1 DESC LIMIT {int(limit_sittings)})
        """
    ...
    log.info("ns_votes: scanned=%d inserted=%d updated=%d skipped=%d by_type=%s by_result=%s bill_links=%d",
