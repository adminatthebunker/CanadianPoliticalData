"""MB votes extractor — Hansard-text regex over already-ingested MB speeches.

Pattern coverage (probe 2026-04-30): 1,356 inline motion outcomes,
2,168 division calls, 3,393 question-call phrases, 0 numerical tallies.
Predominantly consensus-shape.

Idempotency: `(source_system='votes-mb', source_url)`.
"""
...
_QUESTION_CALL_RE = re.compile(
    ...
    r'is it the pleasure of the House|division called|recorded vote)\b',
    re.IGNORECASE,
)
...
def _find_bill_id_for(raw: dict, body: str, idx: dict[str, str]) -> Optional[str]:
    if not idx:
        return None
    topic = (raw or {}).get("mb_hansard", {}).get("topic") or ""
...
async def _load_bill_index(db: Database) -> dict[str, str]:
    rows = await db.fetch(
        ...
        "WHERE ls.province_territory='MB' AND ls.level='provincial'"
    )
    out: dict[str, str] = {}
    for r in rows:
...
async def _upsert_vote(db: Database, *, speech_row: dict, detected: _Detected,
    ...
    result = await db.fetchrow(
        ...
        VALUES ($1::uuid, 'provincial', 'MB', $2, $3::uuid, $4, $5, $6,
...
async def extract_mb_votes(db: Database, *, limit_sittings: Optional[int] = None) -> IngestStats:
    stats = IngestStats()
    bill_index = await _load_bill_index(db)
    log.info("mb_votes: loaded %d MB bill references", len(bill_index))
    where_sittings = ""
    if limit_sittings is not None:
        where_sittings = f"""
        AND s.spoken_at::date IN (SELECT spoken_at::date FROM speeches
          WHERE source_system='hansard-mb' GROUP BY 1 ORDER BY 1 DESC LIMIT {int(limit_sittings)})
        """
    ...
    log.info("mb_votes: scanned=%d inserted=%d updated=%d skipped=%d by_type=%s by_result=%s bill_links=%d",
