"""NL votes extractor — Hansard-text regex over already-ingested NL speeches.

Pattern coverage (probe 2026-04-30): 482 inline motion outcomes,
17 small numerical nay tallies, 19 division calls, 1,110 question-call.
Mostly consensus-shape with rare division-shape.

Idempotency: `(source_system='votes-nl', source_url)`.
"""
...
def _classify(text: str) -> Optional[_Detected]:
    ...
    if tally:
        ...
        sentences = re.split(r'(?<=[.!?])\s+', text[:tally.start()].strip())
        motion = " ".join(sentences[-3:])[:500] if sentences else None
        return _Detected(vote_type="division", result=result, motion_text=motion,
                         ayes=ayes, nays=nays)
...
def _find_bill_id_for(raw: dict, body: str, idx: dict[str, str]) -> Optional[str]:
    if not idx:
        return None
    topic = (raw or {}).get("nl_hansard", {}).get("topic") or ""
...
async def _load_bill_index(db: Database) -> dict[str, str]:
    rows = await db.fetch(
        ...
        "WHERE ls.province_territory='NL' AND ls.level='provincial'"
    )
    out: dict[str, str] = {}
    for r in rows:
...
async def _upsert_vote(db: Database, *, speech_row: dict, detected: _Detected,
    ...
    result = await db.fetchrow(
        ...
        VALUES ($1::uuid, 'provincial', 'NL', $2, $3::uuid, $4, $5, $6,
...
async def extract_nl_votes(db: Database, *, limit_sittings: Optional[int] = None) -> IngestStats:
    stats = IngestStats()
    bill_index = await _load_bill_index(db)
    log.info("nl_votes: loaded %d NL bill references", len(bill_index))
    where_sittings = ""
    if limit_sittings is not None:
        where_sittings = f"""
        AND s.spoken_at::date IN (SELECT spoken_at::date FROM speeches
          WHERE source_system='hansard-nl' GROUP BY 1 ORDER BY 1 DESC LIMIT {int(limit_sittings)})
        """
    ...
    log.info("nl_votes: scanned=%d inserted=%d updated=%d skipped=%d by_type=%s by_result=%s bill_links=%d",
