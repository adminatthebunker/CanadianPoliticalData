...
const COMMAND_CATALOG = [
  ...
  { key: "ingest-federal-committees", category: "hansard",
    description: "Pull federal House of Commons committee evidence (transcripts) from openparliament.ca; lands with speech_type='committee'.",
    args: [
      { name: "parliament", type: "int", required: false, help: "Parliament number (e.g. 45)." },
      { name: "session", type: "int", required: false, help: "Session within the parliament." },
      { name: "since", type: "date", required: false, help: "Only fetch meetings on/after this date." },
      { name: "since_days", type: "int", required: false, help: "Forward-incremental: clamp --since to today - N days." },
      { name: "until", type: "date", required: false, help: "Only fetch meetings on/before this date." },
  ...
  { key: "ingest-ab-committees", category: "hansard",
    ...
    args: [
      { name: "legislature", type: "int", required: false, help: "AB Legislature number (e.g. 31)." },
      { name: "session", type: "int", required: false, help: "Session within the legislature (e.g. 2)." },
      { name: "since", type: "date", required: false, help: "Only fetch meetings on/after this date." },
      { name: "since_days", type: "int", required: false, help: "Forward-incremental: clamp --since to today - N days." },
      { name: "until", type: "date", required: false, help: "Only fetch meetings on/before this date." },
      { name: "limit_meetings", type: "int", required: false, help: "Cap on meeting PDFs fetched." },
      { name: "limit_speeches", type: "int", required: false, help: "Cap on TOTAL speeches ingested." },
      { name: "committees", type: "string", required: false, help: "Comma-separated committee acronyms (e.g. 'HS,EF'). Default: all 11." },
    ],
  },
  ...
  { key: "check-bc-committees-freshness", category: "maintenance",
    description: "SUPERSEDED 2026-07-30: ingest-bc-committees now auto-discovers meetings from the pcms API; the weekly schedule was disabled. Kept for manual audits: per BC standing committee, report days since last meeting in our DB (emails admin past the threshold, default 21 days).",
    args: [
  ...
  { key: "apply-on-municipal-roster", category: "maintenance",
    ...
    args: [
      { name: "cycle", type: "int", required: false, help: "AMO election cycle to write from. Default 2022." },
      { name: "apply", type: "bool", required: false, help: "Actually write. Without it the run only reports its plan." },
    ],
  },
  ...
  { key: "compare-on-municipal-roster", category: "maintenance",
    ...
    args: [
      { name: "cycle", type: "int", required: false, help: "AMO election cycle: 2018, 2022 or 2026. Default 2026." },
    ],
  },
  { key: "reattach-municipal-roster", category: "maintenance",
    ...
    args: [
      { name: "council", type: "str", required: false, help: "Limit to one council slug (middle field of source_id)." },
      { name: "dry_run", type: "bool", required: false, help: "Report without writing." },
    ],
  },
  ...
  { key: "ingest-bc-committee-membership", category: "enrichment",
    ...
    args: [
      { name: "parliament", type: "int", required: false, help: "Historical mode: BC parliament number (e.g. 42) — lands dated rows for the backfill's date-aware lookup." },
    ],
  },
  { key: "ingest-bc-committees", category: "hansard",
    ...
    args: [
      { name: "parliament", type: "int", required: false, help: "BC Parliament number (e.g. 43)." },
      { name: "session", type: "int", required: false, help: "Session within the parliament (e.g. 1)." },
      { name: "since", type: "date", required: false, help: "Only fetch meetings on/after this date." },
      { name: "since_days", type: "int", required: false, help: "Forward-incremental: clamp --since to today - N days." },
      { name: "until", type: "date", required: false, help: "Only fetch meetings on/before this date." },
      { name: "limit_meetings", type: "int", required: false, help: "Cap on meetings fetched (newest-first)." },
      { name: "limit_speeches", type: "int", required: false, help: "Cap on TOTAL speeches ingested." },
      { name: "committees", type: "string", required: false, help: "Comma-separated committee codes (e.g. 'fgs,cay'). Default: all discovered." },
      { name: "seed_file", type: "string", required: false, help: "Seed JSON path (used with use_seed). Default: scripts/seeds/bc-committee-meetings.json." },
      { name: "use_seed", type: "bool", required: false, help: "Read the legacy seed file instead of discovering from the pcms API." },
      { name: "max_pages", type: "int", required: false, help: "Cap pcms discovery pages (50 meetings/page date-desc)." },
    ],
  },
  ...
  { key: "resolve-role-only-presiding-officers", category: "hansard",
    ...
    args: [
      { name: "province", type: "str", required: false,
        help: "2-letter code; default runs every province with a role-only roster. Rosters exist for AB / BC / MB / NS / SK." },
      { name: "limit", type: "int", required: false, help: "Cap candidate speeches scanned." },
    ],
  },
  ...
  { key: "ingest-nt-hansard", category: "hansard", description: "Ingest NT Hansard from ntlegislativeassembly.ca. Discovery via paginated /documents-proceedings/hansard; per-sitting HTML at /hansard/hn{YYMMDD}. Speaker attribution by direct nt_mla_slug FK. Consensus government — party_at_time=NULL.",
    args: [
      ...
      { name: "since_days", type: "int", required: false, help: "Forward-incremental: derive hn_id from today - N days (e.g. 14 for daily schedules)." },
  ...
  { key: "extract-federal-votes", category: "hansard", description: "Extract federal votes + per-MP positions from openparliament.ca structured JSON. Politician FK by openparliament_slug exact match; bill FK by bill_url. vote_type='division' with populated tallies and vote_positions. Idempotent.",
    args: [
      ...
      { name: "since", type: "str", required: false, help: "Forward-incremental: only process votes on/after this ISO date. Wins over --since-days and DB high-water." },
      { name: "since_days", type: "int", required: false, help: "Forward-incremental: only process votes from the last N days (takes precedence over DB high-water)." },
    ],
  },
  ...
  // enrichment
  // ⛔ The Open North roster commands were removed from this catalogue
  // 2026-08-27, in step with services/scanner/src/jobs_catalog.py. The two
  // halves must stay in sync — a command here but not there is refused by the
  // worker as `unknown command`, and a command in BOTH is queueable by hand
  // from the admin panel. Leaving them queueable is what made a retired
  // ingester a one-click revert of twelve cutover migrations.
  ...
  { key: "ingest-nb-former-mlas", category: "enrichment",
    ...
    args: [] },
  { key: "ingest-nu-former-mlas", category: "enrichment",
  ...
  { key: "ingest-qc-former-mnas", category: "enrichment",
    ...
    args: [
      ...
      { name: "bio_for_existing", type: "bool", required: false, default: false, help: "Re-fetch bios for MNAs already in the DB (default: skip). Use after a regex-improvement to widen career spans." },
    ] },
  ...
  { key: "ingest-edmonton-meetings", category: "bills",
  ...
  { key: "match-meetings-to-youtube", category: "hansard",
    ...
    args: [
      { name: "city", type: "enum", required: false, default: "all",
        choices: ["calgary", "edmonton", "all"] },
      { name: "limit", type: "int", required: false },
      { name: "max_channel_videos", type: "int", required: false, default: 800, help: "Newest N videos to consider per channel." },
    ],
  },
  ...
  { key: "reparse-meeting-captions", category: "hansard",
  ...
  { key: "ocr-speaker-timeline", category: "hansard",
    description: "Stage 8 — OCR the clerk's on-screen speaker panel into meetings.raw->'speaker_timeline'. Media comes from the derivative cache (ISI-first source ladder, one fetch per meeting ever); a serial producer feeds concurrent OCR workers. ~10-15 CPU-min per meeting, pipelined.",
    args: [
  ...
  { key: "enrich-edmonton-minutes", category: "hansard",
    ...
    args: [
      { name: "limit", type: "int", required: false, help: "Max meetings to process (default: all video-matched)." },
      { name: "delay", type: "float", required: false, default: 1.0, help: "Seconds between per-meeting minutes fetches." },
    ],
  },
];
